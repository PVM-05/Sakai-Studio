...
class Stack(object):
    ...
    def __call__(self, img_group):
        img_group_copy = list(img_group)
        for i in range(len(img_group_copy)):
            if img_group_copy[i].ndim==3:
                img_group_copy[i] = Image.fromarray(cv2.cvtColor(img_group_copy[i].astype(np.uint8), cv2.COLOR_BGR2RGB))
            elif img_group_copy[i].ndim==2:
                img_group_copy[i] = Image.fromarray(img_group_copy[i].astype(np.uint8))

        mode = img_group_copy[0].mode
        if mode == '1':
            img_group_copy = [img.convert('L') for img in img_group_copy]
            mode = 'L'
        if mode == 'L':
            return np.stack([np.expand_dims(x, 2) for x in img_group_copy], axis=2)
        elif mode == 'RGB':
            if self.roll:
                return np.stack([np.array(x)[:, :, ::-1] for x in img_group_copy], axis=2)
            else:
                return np.stack(img_group_copy, axis=2)
        else:
