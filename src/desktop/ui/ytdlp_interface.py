    ...
    def run(self):
        ...
        if self.custom_filename and self.custom_filename.strip():
            ...
            if final_name != self.custom_filename.strip():
                self.log_sig.emit(f"Tệp '{self.custom_filename.strip()}' đã tồn tại trong thư mục lưu. Đã tự động đổi tên thành: '{final_name}'")
...
class YtdlpInterface(ScrollArea):
    ...
    def setup_ui(self):
        ...
        saved_cookie = self.settings.get('cookies_file', '')
        default_cookie = os.path.join(os.getcwd(), 'cookies.txt')
        if not saved_cookie and os.path.exists(default_cookie):
            saved_cookie = default_cookie
            self.settings['cookies_file'] = saved_cookie
            save_settings(self.settings)
        elif saved_cookie and not os.path.exists(saved_cookie) and os.path.exists(default_cookie):
            saved_cookie = default_cookie
            self.settings['cookies_file'] = saved_cookie
            save_settings(self.settings)
            
        self.cookies_input.setText(saved_cookie)
        ...
        # Cải tiến: Tự động xuống dòng cho các nhãn văn bản
