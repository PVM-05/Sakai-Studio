...
from src.core.tools.common_tools import is_image_file
...
class TaskListComponent(QWidget):
    ...
    def add_task(self, video_path):
        ...
        # Tự động kế thừa tùy chọn vùng chọn phụ đề (SUB_AREAS) và phân đoạn A/B (AB_SECTIONS) từ nhiệm vụ hiện tại hoặc trước đó
        default_options = {}
        if self.tasks:
    ...
    def show_context_menu(self, pos):
        ...
        if index.isValid():
            ...
            # Áp dụng vùng xóa phụ đề cho tất cả các nhiệm vụ
            ...
            def apply_areas_to_all():
                ...
                if not sub_areas:
                    InfoBar.warning(
                        title="Cảnh báo",
                        content="Nhiệm vụ được chọn chưa có vùng phụ đề nào được thiết lập.",
                        parent=self.get_root_parent(),
                        duration=3000
                    )
                    return
