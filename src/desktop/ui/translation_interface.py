...
from src.ai_engines.translator import SubtitleTranslator, parse_srt, blocks_to_srt, SubtitleBlock, fetch_accessible_models
from src.core.tools.subtitle_exporter import frame_to_timestamp_srt
...
class TranslationInterface(QWidget):
    ...
    def _init_ui(self):
        ...
        header_title = TitleLabel("Dịch Phụ Đề", self)
        header_desc = CaptionLabel(
            "Công cụ chuyên biệt dịch phụ đề SRT/VTT/ASS. Hỗ trợ dịch tự động bằng các mô hình AI.", self
        )
        ...
        # Cải tiến: Cho phép nhãn cấu hình tự động xuống dòng tránh bị che khuất
    ...
    def _on_provider_changed(self, index: int):
        ...
        if 0 <= index < len(provider_keys):
            ...
            if info.get("is_gguf"):
                from src.ai_engines.translator import get_available_gguf_models
    ...
    def _open_single_file(self):
        filepath, _ = FolderMemoryDialog.getOpenFileName(
            self,
            "Chọn tệp Phụ đề",
            filter_str="Tệp Phụ đề (*.srt *.vtt *.ass);;Tất cả tệp (*)",
            category="subtitle"
        )
        if filepath:
    ...
    def _on_table_row_selected(self):
        """(Không còn dùng để seek video vì đã tách module)"""
        pass
    ...
    def _populate_table(self, blocks: list[SubtitleBlock]):
        ...
        has_data = len(blocks) > 0
        if hasattr(self, 'translate_btn'):
            self.translate_btn.setEnabled(has_data)
        if hasattr(self, 'export_srt_button'):
            self.export_srt_button.setEnabled(has_data)
    ...
    def _start_translation(self):
        ...
        if not self.subtitle_blocks and not self.current_srt_path:
            InfoBar.warning("Cảnh báo", "Vui lòng mở tệp phụ đề trước khi dịch!", parent=self, duration=3000)
            return
