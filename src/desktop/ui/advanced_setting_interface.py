...
from qfluentwidgets import (ScrollArea, ExpandLayout, FluentIcon,
                           ...
                           SearchLineEdit)
from src.core.config import config, tr, VERSION, PROJECT_HOME_URL, PROJECT_ISSUES_URL, PROJECT_RELEASES_URL, HARDWARD_ACCELERATION_OPTION
from src.core.tools.folder_memory import FolderMemoryDialog
from src.core.tools.version_service import VersionService
from src.core.tools.concurrent import TaskExecutor
...
class AdvancedSettingInterface(QWidget):
    ...
    def setup_ui(self):
        ...
        # ==========================================
        # self.step1_group = SettingCardGroup("Khởi tạo và Khoanh vùng", self.videoScrollWidget.widget())

        self.step2_group = SettingCardGroup("Lựa chọn Mô hình Trí tuệ Nhân tạo", self.videoScrollWidget.widget())
        ...
        self.enable_tensorrt = SwitchSettingCard(
            configItem=config.enableTensorRT,
            icon=FluentIcon.POWER_BUTTON,
            title="Tăng tốc AI bằng TensorRT (NVIDIA)",
            content="Tăng tốc xử lý Xóa phụ đề (LAMA/ProPainter) lên 2-5 lần",
            parent=self.step2_group
        )
        self.enable_tensorrt.setToolTip("Yêu cầu card đồ họa NVIDIA. Lần chạy đầu tiên sẽ mất 3-10 phút để hệ thống biên dịch mô hình phần cứng.")
        ...
        self.step3_group = SettingCardGroup("Cấu hình Chất lượng và Độ nét", self.videoScrollWidget.widget())
        ...
        # ==========================================
        self.srt_feature_group = SettingCardGroup("Trích xuất và Dịch thuật Phụ đề (Tùy chọn)", self.srtScrollWidget.widget())
    ...
    def setup_layout(self):
        ...
        self.step2_group.addSettingCard(self.enable_tensorrt)
    ...
    def apply_preset(self, preset_name: str):
        ...
        if preset_name == "fast":
            config.set(config.autoMaskDilation, False)
            ...
            config.set(config.autoMaskDilation, True)
            config.set(config.hardwareAcceleration, True)
            config.set(config.gpuVideoEncoding, True)
