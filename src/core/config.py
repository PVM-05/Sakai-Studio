...
callers (``from src.core.processor import ProcessingConfig``) keep
working unchanged.
"""
...
class InpaintMode(Enum):
    ...
    # GUI and legacy compatibility aliases
    STTN_AUTO = "sttn-auto"
    STTN_DET = "sttn-det"
    OPENCV = "opencv"
...
class ProcessingConfig:
    ...
    poisson_blending: bool = False  # use Poisson Blending instead of alpha feathering
...
def normalize_processing_config(config: ProcessingConfig) -> ProcessingConfig:
    ...
    config.poisson_blending = _coerce_bool(getattr(config, "poisson_blending", False), False)
...
# =========================================================================
# GUI & Legacy Configuration Compatibility Layer
# =========================================================================
...
from src.core.tools.constant import SubtitleDetectMode, InpaintMode as InpaintModeGUI
import os
import sys
import configparser
...
# Hardware acceleration option switch
...
class Config(QConfig):
    ...
    # Subtitle selection areas
    subtitleSelectionAreas = ConfigItem("Main", "SubtitleSelectionAreas", "0.88,0.99,0.15,0.85")

    # Inpaint Mode
    inpaintMode = OptionsConfigItem("Main", "InpaintMode", InpaintModeGUI.STTN_AUTO, OptionsValidator(InpaintModeGUI), EnumSerializer(InpaintModeGUI))
    ...
    # New Poisson Blending toggle setting
    poissonBlending = ConfigItem("Main", "PoissonBlending", True, BoolValidator())
    temporalSmoothing = ConfigItem("Main", "TemporalSmoothing", True, BoolValidator())
    maskType = OptionsConfigItem("Main", "MaskType", "stroke", OptionsValidator(["stroke", "box"]))
    ...
    autoMaskDilation = ConfigItem("Main", "AutoMaskDilation", True, BoolValidator())
    ...
    maskFadePadding = RangeConfigItem("Main", "MaskFadePadding", 5, RangeValidator(0, 15))
    ...
    trackerAlgorithm = OptionsConfigItem("Main", "TrackerAlgorithm", "csrt", OptionsValidator(["csrt", "kcf", "mil"]))
    burnTranslatedSubtitles = ConfigItem("Main", "BurnTranslatedSubtitles", False, BoolValidator())
    targetLanguage = OptionsConfigItem("Main", "TargetLanguage", "vi", OptionsValidator(["vi", "en", "zh-CN", "ja", "ko", "fr", "de"]))
    exportSrt = ConfigItem("Main", "ExportSrt", False, BoolValidator())
    srtSaveDirectory = ConfigItem("Main", "SrtSaveDirectory", "", ConfigValidator())
    whisperFallback = ConfigItem("Main", "WhisperFallback", False, BoolValidator())
    preserveColorMetadata = ConfigItem("Main", "PreserveColorMetadata", True, BoolValidator())
    ocrMode = OptionsConfigItem("Main", "OcrMode", "auto", OptionsValidator(["auto", "fast", "precise"]))
    ocrLanguage = OptionsConfigItem("Main", "OcrLanguage", "auto", OptionsValidator(["auto", "vi", "en", "ch", "japan", "korean"]))
    mmoHorizontalFlip = ConfigItem("MMO", "HorizontalFlip", False, BoolValidator())
    mmoSmartTextMask = ConfigItem("MMO", "SmartTextMask", False, BoolValidator())
    mmoSpeedMultiplier = ConfigItem("MMO", "SpeedMultiplier", 1.0)
    mmoCropPercentage = RangeConfigItem("MMO", "CropPercentage", 0, RangeValidator(0, 50))
    mmoSubStyle = OptionsConfigItem("MMO", "SubStyle", "tiktok_yellow", OptionsValidator(["tiktok_yellow", "youtube_white", "netflix_classic", "custom"]))
    sam2Refine = ConfigItem("Main", "Sam2Refine", False, BoolValidator())
    voiceSeparation = ConfigItem("Main", "VoiceSeparation", False, BoolValidator())
    autoTighten = ConfigItem("Main", "AutoTighten", False, BoolValidator())

    # --- TTS & Audio Ducking Configs ---
    mmoEnableTTS = ConfigItem("MMO", "EnableTTS", False, BoolValidator())
    mmoAudioDuckingVolume = ConfigItem("MMO", "AudioDuckingVolume", 0.15) # Giảm 85% tiếng gốc
CONFIG_FILE = 'config/config.json'
if not os.path.exists(CONFIG_FILE):
    os.makedirs(os.path.dirname(CONFIG_FILE), exist_ok=True)
    if getattr(sys, 'frozen', False):
        bundled_config = os.path.join(getattr(sys, '_MEIPASS', ''), 'config', 'config.json')
...
# Đưa model của PaddleX vào thư mục dự án
PROJECT_ROOT = str(Path(BASE_DIR).parent.parent)
os.environ['PADDLE_PDX_CACHE_HOME'] = os.path.join(Path(BASE_DIR).parent, 'ai_engines', 'models', 'paddlex')
