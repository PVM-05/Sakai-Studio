...
from src.core.config import VERSION, PROJECT_UPDATE_URLS, tr
    ...
    def get_latest_version(self):
        ...
        for url in self.api_endpoints:
            try:
                ...
                # Nếu là lỗi 404 (Not Found) hoặc lỗi máy chủ/proxy (500, 502) do kho repo chưa có Releases nào
                is_release_not_found = False
                if isinstance(e, requests.exceptions.HTTPError) and e.response is not None:
                    if e.response.status_code in [404, 500, 502]:
                        is_release_not_found = True
                
                # Chỉ in lỗi ra console nếu không phải lỗi chưa tạo release
                if not is_release_not_found:
