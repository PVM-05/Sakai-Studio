...
from src.core.config import tr, config
from src.core.tools.common_tools import get_readable_path
...
class ObjectTracker:
    ...
    def find_subtitle_frame_no(self, sub_remover=None):
        ...
        video_cap = open_capture_no_hwaccel(self.video_path)
        ...
        use_sam2 = getattr(config, 'sam2Refine', None) and config.sam2Refine.value
        if use_sam2:
            try:
                from src.core.tools.sam2_tracker import SAM2VideoTracker
                sam2_tracker = SAM2VideoTracker(self.video_path)
                if getattr(sam2_tracker, 'predictor', None) is not None:
                    video_cap.release()
                    return self._run_sam2_tracking(sam2_tracker, target_boxes, current_frame_no, frame_count, sub_remover, tbar, sub_detector)
            except Exception as e:
                print(f"[ObjectTracker] Khởi tạo SAM2 Video Tracker thất bại, dùng bộ bám đuổi cũ: {e}")
                if sub_remover:
                    sub_remover.append_output("Lỗi khởi tạo SAM 2 Video Tracker, tự động chuyển về Hybrid Tracker.")
        ...
        if current_frame_no > 1:
            if sub_remover:
                sub_remover.append_output(f"Đang tiến hành bám đuổi ngược về khung hình đầu tiên...")
    ...
    def _run_sam2_tracking(self, sam2_tracker, initial_boxes, start_frame, total_frames, sub_remover, tbar, sub_detector):
        import gc
        import torch
        import cv2

        if sub_remover:
            sub_remover.append_output("Đang sử dụng SAM 2 để bám đuổi độ phân giải cao theo cụm.")
        ...
        if start_frame > 1:
            if sub_remover:
                sub_remover.append_output("Đang chạy bám đuổi ngược bằng SAM 2...")
            
            back_boxes = initial_boxes
            curr_back_end = start_frame
            
            while curr_back_end > 1:
                chunk_start = max(1, curr_back_end - chunk_size + 1)
                chunk_end = curr_back_end
                
                if chunk_start == chunk_end:
                    break
                    
                prompt_offset = chunk_end - chunk_start
                
                if sub_remover:
                    sub_remover.append_output(f"Đang lùi (SAM 2) đoạn {chunk_end} -> {chunk_start}...")
                
                try:
                    chunk_dict = sam2_tracker.track_chunk(back_boxes, chunk_start, chunk_end, padding=10, prompt_frame_offset=prompt_offset)
                except Exception as e:
                    print(f"[ObjectTracker] Lỗi SAM 2 Backward chunk {chunk_start}: {e}")
                    break
                    
                if not chunk_dict:
                    break
                    
                subtitle_frame_no_box_dict.update(chunk_dict)
                
                first_boxes_in_chunk = chunk_dict.get(chunk_start, [])
                next_back_boxes = []
                for b in first_boxes_in_chunk:
                    if b is not None and len(b) >= 4:
                        xmin, xmax, ymin, ymax = b[:4]
                        x, y = xmin, ymin
                        w, h = xmax - xmin, ymax - ymin
                        if w >= 5 and h >= 5:
                            next_back_boxes.append((x, y, w, h))
                
                if not next_back_boxes:
                    break
                
                back_boxes = next_back_boxes
                curr_back_end = chunk_start
                
                gc.collect()
                if torch.cuda.is_available(): torch.cuda.empty_cache()
            
            if sub_remover:
                sub_remover.append_output("Hoàn tất bám đuổi ngược. Đang chuyển sang bám đuổi tiến...")

        # 2. FORWARD TRACKING WITH ANTI-DRIFT OCR
        current_boxes = initial_boxes
        video_cap = None
        
        for chunk_start in range(start_frame, total_frames + 1, chunk_size):
            chunk_end = min(chunk_start + chunk_size - 1, total_frames)
            
            # ANTI-DRIFT OCR: Nắn lại toạ độ ở đầu chunk nếu có sub_detector
            if chunk_start > start_frame and sub_detector is not None:
                if video_cap is None:
                    from src.core.tools.common_tools import open_capture_no_hwaccel
                    video_cap = open_capture_no_hwaccel(self.video_path)
                video_cap.set(cv2.CAP_PROP_POS_FRAMES, chunk_start - 1)
                ret_f, frame_f = video_cap.read()
                if ret_f:
                    ocr_boxes = sub_detector.detect_subtitle(frame_f)
                    corrected_boxes = []
                    for (cx, cy, cw, ch) in current_boxes:
                        c_xmin, c_xmax = cx, cx + cw
                        c_ymin, c_ymax = cy, cy + ch
                        matched = False
                        
                        for (oxmin, oxmax, oymin, oymax) in ocr_boxes:
                            inter_xmin = max(c_xmin, oxmin)
                            inter_ymin = max(c_ymin, oymin)
                            inter_xmax = min(c_xmax, oxmax)
                            inter_ymax = min(c_ymax, oymax)
                            inter_w = max(0, inter_xmax - inter_xmin)
                            inter_h = max(0, inter_ymax - inter_ymin)
                            
                            iou = (inter_w * inter_h) / float(cw * ch + (oxmax - oxmin) * (oymax - oymin) - inter_w * inter_h)
                            if iou > 0.15:
                                nx = int(max(0, min(oxmin, frame_f.shape[1] - 5)))
                                ny = int(max(0, min(oymin, frame_f.shape[0] - 5)))
                                nw = int(max(5, min(oxmax - oxmin, frame_f.shape[1] - nx)))
                                nh = int(max(5, min(oymax - oymin, frame_f.shape[0] - ny)))
                                corrected_boxes.append((nx, ny, nw, nh))
                                matched = True
                                break
                        
                        if not matched:
                            corrected_boxes.append((cx, cy, cw, ch))
                            
                    current_boxes = corrected_boxes
            
            if sub_remover:
                sub_remover.append_output(f"Đang bám đuổi tiến (SAM 2) đoạn {chunk_start} -> {chunk_end}...")
            
            try:
                chunk_dict = sam2_tracker.track_chunk(current_boxes, chunk_start, chunk_end, padding=10, prompt_frame_offset=0)
            except Exception as e:
                print(f"[ObjectTracker] Lỗi khi xử lý chunk {chunk_start}: {e}")
                if sub_remover:
                    sub_remover.append_output(f"[Lỗi SAM 2] {e}")
                break
            ...
            tbar.update(chunk_end - chunk_start + 1)
            if sub_remover:
                sub_remover.current_frame_no = chunk_end
                sub_remover.progress_total = int((float(chunk_end) / float(total_frames)) * 40)
                if hasattr(sub_remover, 'notify_progress_listeners'):
                    sub_remover.notify_progress_listeners()
                elif hasattr(sub_remover, 'progress_signal'):
                    sub_remover.progress_signal.emit(sub_remover.progress_total, False, chunk_end)
            
            last_boxes = chunk_dict.get(chunk_end, [])
            next_boxes = []
            for b in last_boxes:
                if b is not None and len(b) >= 4:
                    xmin, xmax, ymin, ymax = b[:4]
            ...
            if not next_boxes:
                if sub_remover:
                    sub_remover.append_output("SAM 2 đã mất dấu hoàn toàn đối tượng, dừng bám đuổi tiến.")
                break
        ...
        if sub_remover:
            sub_remover.append_output("Hoàn tất bám đuổi vị trí đối tượng bằng SAM 2.")
