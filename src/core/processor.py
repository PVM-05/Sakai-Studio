...
re-exported below so legacy callers (``from src.core.processor import
...
# (`from src.core.processor import _open_capture`) keep working.
...
from src.video_ops.encoder import _detect_hw_encoder
...
from src.core.quality_gate import evaluate_quality_gate
from src.core.resume_checkpoint import (
    ProcessingPaused,
...
from src.core.safe_image import safe_imread
from src.video_ops.tracking import (
    _KalmanBox,
    _box_from_state,
    _iou,
    SubtitleTracker,
...
from src.ai_engines.detection import SubtitleDetector, _surya_allowed
from src.ai_engines.inpainters import (
    BaseInpainter,
    STTNInpainter,
    LAMAInpainter,
    ProPainterInpainter,
...
try:
    from src.ai_engines.inpainters.external import ExternalInpainter, is_available as _ext_available
    if _ext_available():
...
class ProSubtitleRemover:
    ...
    def _refine_masks_with_matanyone(self,
        ...
        if not frames or not masks:
            return masks
        try:
            from src.ai_engines.segmentation import refine_masks_with_matanyone
    ...
    def _propagate_masks_with_cotracker(self,
        ...
        if not frames or not masks:
            return masks
        try:
            from src.ai_engines.segmentation import propagate_masks_with_cotracker
    ...
    def _inpaint_with_optional_rife_fast(self,
        ...
        try:
            from src.video_ops.decode_accel import maybe_interpolate_pair
        except Exception as exc:
    ...
    def process_video(self, input_path: str, output_path: str, *,
        ...
        try:
            ...
            if not Path(input_path).is_dir():
                try:
                    from src.video_ops.hdr import probe_color_metadata as _probe
            ...
            if (self.config.preserve_color_metadata
                ...
                try:
                    from src.video_ops.hdr import probe_color_metadata
            ...
            while True:
                    ...
                    if reuse_by_phash or reuse_by_keyframe:
                        ...
                        with self._time_stage("ocr"):
                            if self.config.detection_denoise:
                                try:
                                    from src.video_ops.preprocess import fastdvdnet_denoise_frame
                ...
                with self._time_stage("mask"):
                    if hasattr(self.config, 'maskFadePadding') and self.config.maskFadePadding.value > 0:
                        from src.core.tools.inpaint_tools import apply_temporal_mask_padding
                        masks = apply_temporal_mask_padding(masks, self.config.maskFadePadding.value)
    ...
    def _write_reproducibility_sidecar(
        ...
        *,
        ...
        try:
            from src.core.batch_report import write_output_sidecar
            from src.core.config import VERSION as APP_VERSION
        except Exception:
            return
        try:
    ...
    def _run_post_restore_passes(self, output_path: str, temp_dir: str) -> None:
        ...
        if self.config.upscale_factor in (2, 3, 4):
            try:
                from src.video_ops.post_restore import realesrgan_upscale
        ...
        if self.config.swinir_restore:
            try:
                from src.video_ops.post_restore import swinir_restore
        ...
        if self.config.seedvr2_restore:
            try:
                from src.video_ops.post_restore import seedvr2_restore
        ...
        if self.config.film_grain_strength > 0.0:
            if self._uses_native_av1_film_grain():
                ...
                try:
                    from src.video_ops.post_restore import add_film_grain
        ...
        if self.config.restyle_subtitle:
            try:
                from src.video_ops.post_restore import burn_subtitles
        ...
        if self.config.watermark_image:
            try:
                from src.video_ops.post_restore import burn_watermark
    ...
    def _effective_output_codec(self) -> str:
        ...
        if not getattr(self.config, "preserve_color_metadata", True):
            return requested
        try:
            from src.video_ops.hdr import hdr_safe_codec
    ...
    def _hdr_pixel_format_args(self, codec: str, *, hardware: bool = False) -> List[str]:
        if not self.config.preserve_color_metadata or self._color_metadata is None:
            return []
        try:
            from src.video_ops.hdr import hdr_pixel_format_args
    ...
    def _hdr_encoder_private_args(self, codec: str) -> List[str]:
        if not self.config.preserve_color_metadata or self._color_metadata is None:
            return []
        try:
            from src.video_ops.hdr import hdr_encoder_private_args
    ...
    def _hdr_encode_args(self) -> List[str]:
        ...
        if not self.config.preserve_color_metadata or self._color_metadata is None:
            return []
        try:
            from src.video_ops.hdr import hdr_encode_args
...
if __name__ == "__main__":
    ...
    from src.core.cli import main as _cli_main
    _cli_main()
