...
def _load_runtime_helpers() -> None:
    ...
    from src.core.batch_report import (
        STATUS_CANCELLED,
    ...
    from src.video_ops.remux import (
...
def _print_output_quality_preflight(preflight: dict) -> None:
    from src.core.output_quality_preflight import output_quality_preflight_messages
...
def main():
    ...
    if early_args.support_bundle:
        from src.core.support_bundle import create_support_bundle
    ...
    if args.list_presets:
        from src.core.presets import BUILTIN_PRESETS as _BUILTIN, load_user_presets as _load_user
        rows = []
    ...
    if args.audit_onnx:
        from src.ai_engines.onnx_model_info import print_audit_report
        print_audit_report()
        sys.exit(0)

    if args.audit_windows_ml:
        from src.ai_engines.onnx_model_info import print_windows_ml_probe_report
        print_windows_ml_probe_report()
        sys.exit(0)

    if args.scan_weights:
        from src.ai_engines.model_hashes import print_weight_report
        print_weight_report()
        sys.exit(0)

    if args.cache_info:
        from src.core.cache_inventory import print_cache_info
        print_cache_info()
        sys.exit(0)

    if args.cache_clean:
        from src.core.cache_inventory import clean_cache
    ...
    if args.model_cache_export:
        from src.core.cache_inventory import export_model_cache_bundle
        try:
    ...
    if args.model_cache_import:
        from src.core.cache_inventory import import_model_cache_bundle
        try:
    ...
    if args.self_test:
        from src.core.support_bundle import run_self_test
    ...
    if args.intent:
        from src.core.presets import parse_intent
    ...
    if args.auto_lang_probe:
        ...
        if frame is None:
            ...
            sys.exit(1)
        from src.ai_engines.detection import probe_language
    ...
    if args.preset:
        from src.core.presets import preset_fields as _preset_fields
    ...
    remover = ProSubtitleRemover(config)
    ...
    if args.nle_input:
        from src.video_ops.nle_sidecar import parse_nle_input
        raw_fps = cap_fps = 24.0
        try:
            from src.core.io import _probe_duration_seconds
            import cv2 as _cv2
