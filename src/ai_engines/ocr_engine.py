...
class VideoOcrEngine:
    ...
    def _normalize_sub_areas(self, sub_areas, width: int, height: int) -> tuple:
        ...
        if not sub_areas or (isinstance(sub_areas, (list, tuple)) and len(sub_areas) == 0):
            # Quét toàn bộ khung hình nếu người dùng không khoanh vùng
    ...
    def _load_ocr_engine(self):
        """Load OCR engine dựa trên cấu hình (RapidOCR, PaddleX hoặc PaddleOCR)."""
        ...
        if mode_val == SubtitleDetectMode.PADDLE_OCR.value:
            ...
            try:
                ...
                logger.info("Main OCR khởi tạo với PaddleOCR (cổ điển)")
                return paddle_engine
            except Exception as e:
                logger.error(f"Lỗi khởi tạo PaddleOCR: {e}. Đang chuyển sang RapidOCR.")
            ...
            try:
                ...
                logger.info("Main OCR khởi tạo với PaddleX")
                return paddle_engine
            except Exception as e:
                logger.error(f"Lỗi khởi tạo PaddleX: {e}. Đang chuyển sang RapidOCR.")
        
        # Fallback hoặc chọn RapidOCR
        try:
            ...
            if use_tensorrt:
                ...
                logger.info("Main OCR khởi tạo với RapidOCR (TensorRT)")
            elif use_cuda:
                params["EngineConfig.onnxruntime.use_cuda"] = True
                params["EngineConfig.onnxruntime.cuda_ep_cfg.device_id"] = 0
                logger.info("Main OCR khởi tạo với RapidOCR (CUDA)")
        except ImportError:
            params = {}
    ...
    def _ocr_single_frame(self, ocr, crop: np.ndarray) -> str:
        ...
        try:
            ...
            if mode_val != SubtitleDetectMode.RAPID_OCR.value and hasattr(ocr, "ocr"):
                res = ocr.ocr(crop, det=True, rec=True)
            else:
                res = ocr(crop)
            ...
            if res:
                if mode_val != SubtitleDetectMode.RAPID_OCR.value and hasattr(ocr, "ocr"):
                    # PaddleOCR format: [[[box, (text, score)], ...]]
                    if isinstance(res, (list, tuple)) and len(res) > 0 and res[0]:
                        for item in res[0]:
                            if item and len(item) > 1 and isinstance(item[1], (list, tuple)) and item[1][0]:
                                lines.append(str(item[1][0]).strip())
                else:
                    # RapidOCR format
                    if hasattr(res, "txts") and res.txts is not None:
                        for txt in res.txts:
                            if txt:
                                lines.append(str(txt).strip())
                    elif isinstance(res, (list, tuple)) and len(res) > 0 and res[0]:
                        for item in res[0]:
