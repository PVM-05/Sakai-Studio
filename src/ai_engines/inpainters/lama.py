...
from src.core.import_safety import module_can_import as _module_can_import
...
def _try_onnx_session(model_path: str, device: str, enable_tensorrt: bool = False):
    ...
    try:
        from src.ai_engines.inpainters_onnx import (
        ...
        providers = _providers_after_opset_audit(
            model_path, _providers_for_device(device, enable_tensorrt)
        )
...
class LAMAInpainter(BaseInpainter):
    ...
    def _load_model(self):
        ...
        if onnx_path:
            session, provider = _try_onnx_session(onnx_path, self.device, getattr(self.config, 'enable_tensorrt', False))
    ...
    def _verify_pytorch_weights(self):
        try:
            ...
            from src.ai_engines.model_hashes import (
