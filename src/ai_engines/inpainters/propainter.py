...
from src.ai_engines.inpainters.lama import (
    _module_can_import,
...
class ProPainterInpainter(BaseInpainter):
    ...
    def __init__(self, device: str = "cuda:0", config=None):
        ...
        try:
            ...
            if getattr(self.config, 'enable_tensorrt', False):
                try:
                    import torch
                    import torch_tensorrt
                    # Requires PyTorch >= 2.0 and torch-tensorrt
                    self._lama = torch.compile(self._lama, backend="tensorrt")
                    logger.info("Successfully compiled PyTorch LaMa with torch-tensorrt")
                except ImportError:
                    logger.warning("torch-tensorrt not found. Install it to enable TensorRT compilation for PyTorch models.")
                except Exception as e:
                    logger.warning("torch.compile with tensorrt backend failed: %s", e)
        except Exception:
            pass
