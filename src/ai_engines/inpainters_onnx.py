...
def _maybe_session(model_path: str, providers=None, adapter_name: str = "lama-onnx"):
    ...
    if not Path(model_path).is_file():
        ...
        return None
    try:
        from src.core.adapter_manifest import (
    ...
    try:
        ...
        from src.ai_engines.onnxruntime_cuda import preload_onnxruntime_cuda_dlls_if_needed
...
def _providers_for_device(device: str, enable_tensorrt: bool = False) -> List:
    ...
    if "cuda" in device:
        ...
        if enable_tensorrt:
            import os
            os.makedirs("./models/trt_cache", exist_ok=True)
            providers.append(
                (
...
def windows_ml_probe_decision(run_smoke: bool = False) -> dict:
    ...
    try:
        from src.ai_engines.onnx_model_info import collect_windows_ml_probe
...
def _providers_after_opset_audit(model_path: str, providers: List) -> List:
    ...
    if not any(_provider_name(p) == "DmlExecutionProvider" for p in providers):
        return providers
    try:
        from src.ai_engines.onnx_model_info import (
...
class LamaOnnxInpainter:
    ...
    def __init__(self, device: str = "cpu", config=None):
        ...
        providers = []
        try:
            from src.ai_engines.tensorrt_compile import (
...
def _apply_feather_blend(original, filled, masks, config):
    ...
    if config is None:
        return list(filled)
    try:
        from src.ai_engines.inpainters import _feather_blend, _edge_ring_color_correct
    except Exception:
        return list(filled)
    out = []
...
def maybe_register() -> List[str]:
    ...
    from src.ai_engines.inpainter_registry import register
    registered = []
