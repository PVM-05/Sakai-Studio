...
def build_paddleocr(lang: str, device: str, **extra):
    ...
    use_cuda = "cuda" in device or "gpu" in device
    ...
    if is_v3:
        logger.info(f"PaddleOCR {paddle_version} loaded (3.x API, lang={lang})")
    else:
        logger.info(f"PaddleOCR {paddle_version} loaded (2.x API, lang={lang})")
